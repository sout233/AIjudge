...
import { Upload, FileJson, Info, Loader2, FileText, Layout, Code, Plus, Trash2 } from 'lucide-react';
...
export function RuleManagement() {
  ...
  const { data: ruleData, isLoading: loadingRule } = useQuery({
    ...
    queryFn: () => adminApi.getRule(selectedId),
    enabled: !!selectedId,
  });
  ...
  const updateDimension = (dIdx: number, field: string, value: any) => {
    const data = getParsedContent();
    data.dimensions[dIdx][field] = value;
    updateContent(data);
  };
  ...
  const updatePoint = (dIdx: number, pIdx: number, field: string, value: any) => {
    const data = getParsedContent();
    data.dimensions[dIdx].points[pIdx][field] = value;
    updateContent(data);
  };
  ...
  const addPoint = (dIdx: number) => {
    ...
    data.dimensions[dIdx].points.push({
      point_name: "新评分项",
      max_score: 0,
      score_levels: { "优秀": [0, 0], "良好": [0, 0], "一般": [0, 0], "差": [0, 0] }
    });
    updateContent(data);
  };
  ...
  const updateLevel = (dIdx: number, pIdx: number, level: string, boundIdx: number, value: string) => {
    const data = getParsedContent();
    const val = parseFloat(value) || 0;
    data.dimensions[dIdx].points[pIdx].score_levels[level][boundIdx] = val;
    updateContent(data);
  };

  // --- 其他上传逻辑 (保持不变) ---
  const uploadMutation = useMutation({
    mutationFn: ({ id, file }: { id: string; file: File }) => adminApi.saveRule(id, file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rule', selectedId] });
      alert('文件上传成功');
  ...
  const processStandardMutation = useMutation({
    mutationFn: (file: File) => adminApi.processScoringStandard(selectedId, file),
    onSuccess: (result) => {
  ...
  const handleProcessStandard = async () => {
    ...
    try {
      // 2. 发起请求 (使用 mutateAsync 方便配合 Promise)
    ...
    <div className="relative space-y-6">
      ...
      {selectedId ? (
                    ...
                    <TabsContent value="visual" className="space-y-4">
                      <div className="flex items-center gap-4 p-3 bg-muted/50 rounded-lg">
                        ...
                        <Input 
                          ...
                          onChange={(e) => updateContent({...parsedData, total_score: Number(e.target.value)})}
                        />
                      </div>

                      {parsedData.dimensions?.map((dim: any, dIdx: number) => (
                        <div key={dIdx} className="border rounded-lg p-4 space-y-4 bg-card shadow-sm">
                          <div className="flex items-center gap-3 pb-3 border-b">
                            ...
                            <div className="flex items-center gap-2 shrink-0 text-xs">
                              <span>权重:</span>
                              <Input type="number" className="w-16 h-8" value={dim.dimension_weight} onChange={(e) => updateDimension(dIdx, 'dimension_weight', Number(e.target.value))} />
                              <span>满分:</span>
                              <Input type="number" className="w-16 h-8" value={dim.dimension_max_score} onChange={(e) => updateDimension(dIdx, 'dimension_max_score', Number(e.target.value))} />
                            </div>
                            <Button variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => removeDimension(dIdx)}>
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>

                          <div className="pl-4 space-y-4 border-l-2 border-primary/20">
                            {dim.points?.map((point: any, pIdx: number) => (
                              <div key={pIdx} className="space-y-2 bg-slate-50/50 p-3 rounded border border-dashed">
                                <div className="flex items-center gap-3">
                                  <Input 
                                    placeholder="评分项名称" 
                                    className="h-8 text-sm" 
                                    value={point.point_name} 
                                    onChange={(e) => updatePoint(dIdx, pIdx, 'point_name', e.target.value)}
                                  />
                                  <div className="flex items-center gap-2 shrink-0 text-xs">
                                    <span>分值:</span>
                                    <Input type="number" className="w-16 h-8" value={point.max_score} onChange={(e) => updatePoint(dIdx, pIdx, 'max_score', Number(e.target.value))} />
                                  </div>
                                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => removePoint(dIdx, pIdx)}>
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                  {LEVELS.map(lvl => (
                                    <div key={lvl} className="space-y-1">
                                      <span className="text-[10px] text-muted-foreground font-medium">{lvl}区间</span>
                                      <div className="flex items-center gap-1">
                                        <Input 
                                          type="number" className="h-7 px-1 text-[10px]" 
                                          value={point.score_levels[lvl][0]} 
                                          onChange={(e) => updateLevel(dIdx, pIdx, lvl, 0, e.target.value)} 
                                        />
                                        <span className="text-xs">-</span>
                                        <Input 
                                          type="number" className="h-7 px-1 text-[10px]" 
                                          value={point.score_levels[lvl][1]} 
                                          onChange={(e) => updateLevel(dIdx, pIdx, lvl, 1, e.target.value)} 
                                        />
                                      </div>
                                    </div>
                                  ))}
                            ...
                            <Button variant="outline" size="sm" className="w-full border-dashed h-8 text-xs" onClick={() => addPoint(dIdx)}>
                      ...
                      <Button variant="outline" className="w-full h-12 border-2 border-dashed" onClick={addDimension}>
        ...
        <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-border rounded-lg">
          ...
          <p className="text-muted-foreground">请先选择竞赛以配置规则</p>
        </div>
      )}
    </div>
  );
