...
// 获取所有赛道列表
function useAllTracks(contests: Contest[]) {
  return useMemo(() => {
    const tracks: Array<{ contest: Contest; track: Track }> = [];
...
export function RuleManagement() {
  ...
  const allTracks = useAllTracks(contests);

  const { data: ruleData, isLoading: loadingRule } = useQuery({
    queryKey: ['rule', selectedTrackId],
    queryFn: () => adminApi.getRule(selectedTrackId),
  ...
  const saveMutation = useMutation({
    mutationFn: ({ trackId, content }: { trackId: string; content: string }) =>
      adminApi.saveRule(trackId, content),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rule', selectedTrackId] });
      alert('规则保存成功');
    },
    onError: () => alert('保存失败'),
  });
  ...
  const handleSaveContent = () => {
    ...
    saveMutation.mutate({ trackId: selectedTrackId, content });
  };
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ trackId, file }: { trackId: string; file: File }) => adminApi.saveRule(trackId, file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['rule', selectedTrackId] });
      alert('文件上传成功');
  ...
  const processStandardMutation = useMutation({
    mutationFn: (file: File) => adminApi.processScoringStandard(selectedTrackId, file),
    onSuccess: (result) => {
  ...
  const handleUpload = () => {
    ...
    uploadMutation.mutate({ trackId: selectedTrackId, file });
  };
  ...
  const selectedTrackInfo = useMemo(() => {
    return allTracks.find((t) => t.track.id === selectedTrackId);
  ...
  return (
    <div className="relative space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium">选择赛道</label>
        <Select value={selectedTrackId} onValueChange={setSelectedTrackId}>
          <SelectTrigger className="max-w-md">
            <SelectValue placeholder="请选择一个赛道..." />
          </SelectTrigger>
          <SelectContent>
        ...
        {selectedTrackInfo && (
          <div className="text-sm text-muted-foreground">
            所属竞赛: <span className="font-medium">{selectedTrackInfo.contest.name}</span>
            {selectedTrackInfo.track.description && (
              <span className="ml-2">- {selectedTrackInfo.track.description}</span>
            )}
          </div>
        )}
      </div>
      ...
      {selectedTrackId ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-border rounded-lg">
          ...
          <p className="text-muted-foreground">请先选择赛道以配置规则</p>
        </div>
      )}
    </div>
  );
